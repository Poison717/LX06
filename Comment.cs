...
namespace DAL
{
        ...
        public int Add(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into Comment(commentID.workId,userName,CommentContent,CommentTime)" + "values(@commentID,@workId,@userName,@CommentContene,@CommentTime);";
                sql += "SELECT @@IDENTITY";
        ...
        public int Update(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update Comment set CommentContent=@CommentContent,CommentTime=@CommentConten where commentID=@commentID";
